...
The case-run drawer surfaces a dedicated **LLM calls** tab that derives a focused list from the trace whenever a case run produced at least one matching span. By default, every span with `kind: 'llm'` is treated as an LLM call and the tab reads `model`, `latencyMs`, `usage.inputTokens`, `usage.outputTokens`, `usage.cachedInputTokens`, `usage.cacheCreationInputTokens`, `usage.cacheCreationInput1hTokens`, `usage.reasoningTokens`, `tokensPerSecond`, `steps`, `finishReason`, `provider`, `input`, `output`, `reasoning`, and `toolCalls` from the span's attributes. `latencyMs` is time to first token; the full elapsed span time is shown separately as duration. Each row is collapsed by default; clicking expands it to show a per-token-type tokens + cost breakdown table, latency, duration, tokens/sec, finish reason, input/output JSON, reasoning text, tool calls, and any custom metrics.
