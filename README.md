...
When a non-actual scenario is selected each LLM call expands to render two breakdown tables side-by-side — `Simulated` on the left, `Actual` on the right — so the cost delta is immediately visible. Only the breakdown tables and the row's header cost chip change; persisted `costUsd` outputs, stats, and charts continue to reflect the recorded billed cost.
