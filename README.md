...
Eval freshness is tracked from the eval source fingerprint captured on the
latest saved run. When you review an eval-only change and decide the latest run
is still acceptable, use **Mark fresh** in the app to update that saved run's
fingerprint without re-running the eval.
