...
3. **Open the UI** — `agent-evals app` serves it at `http://localhost:4100` (override with `--port`). Use the sidebar status counts to filter visible evals by one or more states, or open the **Tags** filter to narrow the list to evals carrying selected tags (the picker is searchable so workspaces with many tags stay compact); both filters can be combined and round-trip through URL search params (`?status=…`, `?tag=…`). Running evals show a blinking dot in navigation and an elapsed-time status badge on the eval page, while queued evals and cases show as enqueued until a worker starts them; app-triggered runs print the targeted eval names, `file#id` references, resolved concurrency, live case-start lines for evals that are actually running, and a short result summary in the terminal running the server. When a run records LLM spans, that summary also reports how many configured LLM calls executed and how many were replayed from the Agent Eval operation cache. On an eval page, the Run chevron can open a case picker for running a selected subset of authored cases; case-picked runs are temporary by default and can be made durable in the modal. The app can also filter the runs table by applicable result or recent-activity buckets, share that filter through the URL, and clear the currently filtered saved runs when needed. The run drawer can copy the saved run folder path for quick artifact inspection and can keep a temporary run as durable history. Case drawer actions can copy the saved case detail path or the saved run folder path. Each eval's actions menu can copy matching CLI run and debug commands using the workspace package manager. The app watches `agent-evals.config.ts` and reloads config in place when idle; if the file changes during a run, the UI shows a pending banner and new runs are blocked until the current run finishes and the reload applies.
...
- `evalAssert(value, message)` accepts any value, narrows it as truthy in TypeScript, and records pass/fail assertion results while an eval case scope is active. Failed assertions are also listed in `assertionFailures` for compatibility and fail the case. Outside a case scope, it is a runtime no-op so shared workflow code can be reused safely.
...
Eval freshness is tracked from the eval source fingerprint captured on the latest saved run. When you review an eval-only change and decide the latest run is still acceptable, use **Mark fresh** in the app to update that saved run's fingerprint without re-running the eval.
...
`columns` populates your custom columns. Global `columns` from `agent-evals.config.ts` apply to every eval, and eval-level `columns` override global metadata for matching keys. Authored columns are ordered before built-in usage columns that are added by default.
...
The case-run drawer surfaces a dedicated **LLM calls** tab that derives a focused list from the trace whenever a case run produced at least one matching span. By default, every span with `kind: 'llm'` is treated as an LLM call and the tab reads `model`, `latencyMs`, `usage.inputTokens`, `usage.outputTokens`, `usage.cachedInputTokens`, `usage.cacheCreationInputTokens`, `usage.cacheCreationInput1hTokens`, `usage.reasoningTokens`, `steps`, `finishReason`, `provider`, `input`, `output`, `reasoning`, and `toolCalls` from the span's attributes. When an LLM span does not have an attribute-backed `steps` array, direct child spans with `kind: 'model_step'` are shown as that call's steps. Tool calls are aggregated from the LLM span's configured `toolCalls` attribute, any tool calls attached to step objects or direct `model_step` child spans, Mastra's serialized `mastra.model_step.output` format, and child `tool_call` execution spans under each model step. `latencyMs` is time to first token; the full elapsed span time is shown separately as duration. Total tokens, output tokens/sec, and USD costs are derived rather than read from span attributes. Each collapsed row highlights the step count when known; clicking expands it to show a per-token-type tokens + cost breakdown table, latency, duration, output tokens/sec, finish reason, simplified chat messages from `input.messages` plus assistant text from `output.text` when present, input/output JSON, reasoning text, steps, tool calls, any custom metrics, and an action that opens the matching span subtree in the Trace tab. Step cards prioritize their input, output, and reasoning values, while the full raw step payload stays minimized until that raw section is expanded or opened fullscreen, so imported `model_step` spans remain readable even when they include provider metadata.

The expanded breakdown table includes separate **Cache write** and **Cache read** rows so providers like Anthropic — which charge a premium for cache creation (1.25× / 2× the base input rate) and a deep discount on cache reads (0.1×) — show up correctly. The breakdown also includes a `Total input` subtotal that sums the input-side rows (input + cache write + cache read), so the cost split between input and output is easy to read at a glance. Token counts are rendered with locale-aware thousand separators. Configure `llmCalls.pricing` once in `agent-evals.config.ts` as an object keyed by exact model names and the UI derives USD costs from token counts, with nested `providers` entries taking precedence for matching `provider` attributes. Cache read/write tokens are reported separately and contribute to USD cost; when deriving the base input cost, the runner subtracts cache read/write tokens from `inputTokens` so those tokens are not billed twice. `cacheCreationInputTokens` is treated as the total cache-write count; optional `cacheCreationInput1hTokens` only splits that total so extended-cache writes can use `cacheCreationInput1hUsdPerMillion` while the remaining write tokens use `cacheCreationInputUsdPerMillion`. `totalTokens` is always input + output tokens because cache read/write and reasoning counts are subsets of those categories. Reasoning tokens are displayed separately when present, but they are not billed again when included in `outputTokens`; `reasoningUsdPerMillion` is only used for providers that report reasoning outside the output-token total. Built-in LLM costs only come from `llmCalls.pricing`; span cost attributes such as `costUsd` are ignored by the LLM calls tab and default usage outputs. Add `llmCalls.costCurrencies` when you want the expanded breakdown table to show converted cost columns next to USD; these conversions only affect that table and do not change persisted USD cost outputs, stats, or charts.

Above the LLM calls list a **Simulate cost** dropdown lets you compare actual costs against a hypothetical billing scenario derived from the same token counts. The set is intentionally minimal — every scenario answers a distinct question:

- **Actual** — recorded billed cost as-is.
- **Without caching** — worst case: bill every input token at the base input rate, no cache discount.
- **Warmed cache (reads only)** — steady-state cost on a fully warmed cache. Cache writes are treated as already paid (free), cache reads keep the cache-read discount, base input keeps the base rate. When the call has no caching at all, every input token is treated as a cache read. Cache-read pricing is the same on the base (5-minute) and extended (1-hour) tiers, so this scenario covers the warmed case for both TTLs.
- **First call · base cache write** — first-call cost paying the base (5-minute) cache write premium. When the call already uses caching, every cache write token is billed at the 5-minute rate (any extended-cache split is folded back). When the call has no caching, every input token is billed at the 5-minute write rate, as if this were the first call warming up the base cache.
- **First call · extended cache write** — first-call cost paying the extended (e.g. 1-hour) cache write premium. When the call already uses caching, every cache write token is billed at the extended rate. When the call has no caching, every input token is billed at the extended write rate, as if this were the first call warming up the extended cache.

When a non-actual scenario is selected each LLM call expands to render two breakdown tables side-by-side — `Simulated` on the left, `Actual` on the right — so the cost delta is immediately visible. Only the breakdown tables and the row's header cost chip change; persisted `costUsd` remains the actual billed cost. The runner also derives normalized eval-level cost outputs for the default history chart, described below.
...
Authored outputs with the same key are never overwritten. Authored `columns`, `stats`, and `charts` also remain authoritative; authored columns are ordered before missing default usage columns, and matching authored default keys keep default formatting unless overridden. Default columns use compact token formatting, dollar formatting for cost outputs, and duration formatting for `llmDurationMs`, while default stats and LLM usage charts are appended after authored config. `costUsdWithoutCache` bills every input token at the base input rate with no cache discount; `costUsdWarmedCache` treats cacheable tokens as warmed cache reads and cache writes as already paid. These normalized cost columns are hidden from the case table by default but feed the LLM cost chart. The web UI also fills in baseline run-health stats (`cases`, `passRate`, `duration`) and a pass-rate/duration history chart when an eval has not already authored equivalent run-health UI, so eval pages still summarize normal runs without LLM/API usage values. If discovery metadata is missing but saved runs contain runtime columns such as `costUsd`, `inputTokens`, or `apiCalls`, the single-eval page can infer the standard usage stats and charts from those saved run values. Default usage columns, stats, and charts use `hideIfNoValue: true`, so the UI stays quiet until matching LLM/API span data exists. Default LLM usage charts render cost, turns, input tokens, and output tokens separately and use `dedupeConsecutiveValues: true` to skip repeated adjacent points such as cached reruns with unchanged chart values. The default LLM cost chart plots three average-cost series: actual billed cost, without-cache normalized cost, and warmed-cache normalized cost. `apiCalls` is counted from spans matched by `apiCalls.kinds`. Cost defaults use `llmCalls.pricing`, exactly like the LLM calls tab. `totalTokens` is always input + output tokens; reasoning tokens are treated as output detail and are not billed again when included in `outputTokens`. `llmTurns` is the maximum per-call turn count in the case run, using configured steps when available and otherwise one turn per matched LLM call span. `llmDurationMs` sums the elapsed durations of matched LLM call spans; it is not time-to-first-token latency.

Run summaries also aggregate configured LLM call spans into `llmCalls`, `llmCallsMade`, and `llmCacheHits`. `llmCacheHits` counts calls whose own span or ancestor span was replayed from the Agent Eval operation cache (`cache.status: "hit"`); provider prompt-cache token reads such as `cachedInputTokens` still count as executed LLM calls because the provider was reached. CLI and app terminal summaries print these counts when a run includes LLM calls.
...
Removing `costUsd` removes the whole default cost family, including the normalized `costUsdWithoutCache` and `costUsdWarmedCache` outputs and chart series.
...
- Authored raw cache keys are stored for debugging in `<workspaceRoot>/.agent-evals/cache-debug/<sanitizedNamespace>/<keyHash>.json`. These debug files also mirror the serialized cache entry for easier inspection. Binary key parts such as `Buffer`, `ArrayBuffer`, and typed arrays are written as compact type, byte-length, and sha256 metadata instead of embedded byte arrays. This folder may contain prompts, user inputs, cached payloads, or other sensitive data, is not needed for cache reuse, and should be gitignored. Normal cache files remain hash-only.
- Each namespace keeps at most `cache.maxBytes` indexed bytes, or `3 * 1024 * 1024` bytes when no cache retention cap is configured. Retention counts cache payload files, raw-key debug sidecars, and referenced external JSON blobs without double-counting blobs within the namespace, then prunes old indexed entries after a run finishes and the runner stays idle for `cache.pruneIdleDelayMs ?? 5000` milliseconds. Cache referenced only by saved runs for deleted evals is removed first. Cache referenced only by non-latest saved runs older than `cache.oldRunMaxAgeMs ?? 1_296_000_000` milliseconds (15 days) is removed even when the namespace is under the byte cap. If the namespace is still over budget, retention prunes older run-referenced entries before falling back to least-recently-accessed entries. Cache referenced by the latest saved run for each current eval is protected from automatic retention pruning. Cache writes initialize `lastAccessedAt` to the entry's `storedAt`, and hits update `lastAccessedAt` in the namespace index at most once every `cache.lastAccessedAtUpdateIntervalMs ?? 14_400_000` milliseconds (four hours). Legacy index rows may still have `lastAccessedAt: null`. Use the object form of `cache.maxBytes` for exact namespace overrides.
...
- Cached payloads use JSON-safe tagged serialization, so return values and recorded SDK effects preserve richer built-ins such as `Date`, `Map`, `Set`, typed arrays, `URL`, `Headers`, `Blob`, and `File` on cache hits. Typed arrays and other array-buffer views are stored as base64-tagged binary data instead of JSON object properties for each byte. File outputs emitted with `setOutput(...)` / `setEvalOutput(...)` inside a cached operation are recorded as `Blob`/`File` values and replayed on hits, then copied into the current run's artifact directory like fresh outputs. Undefined values are omitted by default instead of being written to cache files; callers using `serializeCacheValue(...)` or `serializeCacheRecording(...)` directly can pass `{ preserveUndefined: true }` to retain explicit undefined wrappers. Cache keys still use the deterministic key-hashing rules above.
...
You can also tune cache retention:
...
Store output values with the `setOutput(...)` helper passed to `execute` as plain data: strings, numbers, booleans, `null`, objects/arrays, explicit file refs, or native `Blob`/`File` values. Stored output values are not coerced by column formats; `columns.format` only controls visualization in the CLI/UI. For one-off output display, pass a format string or column override directly as the third argument: `setOutput('response', markdown, 'markdown')` or `setOutput('receipt', file, { label: 'Receipt', format: 'image' })`. Authored global/eval `columns` for the same key take precedence over the runtime `setOutput` display hint. Values that are not plain JSON, such as `Date`, `Map`, `Set`, `BigInt`, typed arrays, or class instances, are persisted through the tagged value serializer instead of a string fallback. Native `Blob`/`File` values, including entries in output arrays, are copied to separate run artifacts because saved run files are JSON. File arrays preserve order and filenames; each item gets its own artifact, even when filenames repeat. For example, `setOutput('screenshots', [beforeScreenshot, afterScreenshot])` accepts files or blobs, and arrays can also contain `repoFile(...)` references. When the eval has a typed outputs generic, `setOutput(...)` keys and values are typed from that output map. Use the global `setEvalOutput(...)` from shared workflow code that does not receive the execute context. Use `incrementEvalOutput(...)` for numeric totals, `appendToEvalOutput(...)` for arrays that preserve existing scalar values, and `mergeEvalOutput(...)` for shallow object updates.
...
Use the eval `columns` option to control labels, authored column order, alignment, visibility, and rendering format. Supported `columns.format` values include `boolean`, `markdown`, `json`, `image`, `html`, `pdf`, `audio`, `video`, `file`, `percent`, `duration`, `number`, `passFail`, and `stars`. Image, HTML, PDF, audio, video, and generic file references render as artifact cards in the case detail view. Arrays of file references render one card per file, infer each file’s preview from its MIME type, and support an explicit format override for the whole array. Media cards open a preview modal that can navigate between previewable artifacts for the same case; generic file cards download the artifact. Cards show file sizes when the output reference includes one, including `Blob` and `File` outputs persisted by the runner.
