...
`manualScores` declares score columns that reviewers fill in after a run. Pending values keep the eval in an `unscored` state instead of failing. Every manual score requires a non-empty `description` explaining what reviewers need to inspect; it appears as a run-table tooltip and visible score-panel guidance.

See `EvalScoreDef` / `EvalManualScoreDef` in the types for the full shape (format, threshold, column overrides).

Reviewers can override an invalid computed score on a finished run without rerunning (`PUT /api/runs/<runId>/cases/<caseId>/score-overrides/<scoreKey>` with `{ "value": 0..1, "reason"?: string }`; `DELETE` reverts). Overridden cases store the new value in `columns[scoreKey]` and `scoreOverrides[scoreKey] = { originalValue, reason?, overriddenAt }` in `cases.jsonl` and case detail JSON; status and run summary are recomputed, and a threw/non-number scorer failure for that key stops gating the case. When analyzing run artifacts, check `scoreOverrides` before treating a score as the scorer's output, and prefer fixing the scorer when the same override keeps recurring. Reruns discard overrides.

## Outputs, columns, trace display

- `setEvalOutput(key, value)` writes reviewable data for the case. Values are stored as received: primitives, objects/arrays, explicit file refs, and native `Blob`/`File` values. `columns.format` only controls visualization. Inside `execute`, `setOutput(key, value, formatOrOverride)` can attach a display hint directly to a runtime output, e.g. `'markdown'` or `{ label: 'Receipt', format: 'image', hideInTable: true }`. Authored global/eval `columns` for the same key take precedence over that runtime hint. Non-JSON runtime values such as `Date`, `Map`, `Set`, `BigInt`, typed arrays, and class instances use the tagged value serializer instead of a string fallback. Native `Blob`/`File` values, including entries in output arrays, are copied to separate run artifacts because saved run files are JSON. File arrays preserve order and filenames; each item gets its own artifact, even when filenames repeat. For example, `setOutput('screenshots', [beforeScreenshot, afterScreenshot])` accepts files or blobs, and arrays can also contain `repoFile(...)` references. Inside `execute`, prefer the context `setOutput(key, value)` helper when writing schema-backed outputs; it is typed from the eval's outputs generic. Keep `setEvalOutput` for shared workflow code that does not receive the execute context.
...
- `columns` overrides the display for output and score keys (label, description, format, alignment, visibility). The set of supported formats is declared by the `ColumnFormat` union and `EvalColumnOverride` in the types. Global `columns` in `agent-evals.config.ts` apply to every eval; eval-level `columns` override matching global keys. Authored columns are ordered before built-in usage columns that are added by default. Use `hideIfNoValue: true` to hide a column when every row is missing the value, `null`, or an empty string; `0` and `false` still count as values. Use `format: 'image'`, `'html'`, `'pdf'`, `'audio'`, `'video'`, or `'file'` for `Blob`/`File` outputs or `repoFile(...)` references that should render as reviewable artifacts. Persisted `Blob`/`File` artifacts include byte sizes in their run artifact refs; pass the optional `repoFile(..., ..., sizeBytes)` hint when a repository file card should show a size.
- `inputSections` highlights important case input values in the Input tab. Configure it globally or per eval as `{ key: 'prompt.data.test' }`, `{ key: (input) => value }`, or `{ key: { path: 'visual_reference_urls', label: 'Visual references', format: 'pdf' } }`; object entries also accept `select`, `format`, `numberFormat`, and `maxStars`. Eval-level keys override global keys. Selected values use the same rich formats as outputs. Selected `file://` URL strings, including arrays of them, are copied into run artifacts and rendered as files/media when their extension identifies a supported MIME type.
...
- When an eval-only source change has been reviewed and the latest saved run is still acceptable, the app can mark that latest run as current without re-running; this updates only the persisted eval source fingerprint for freshness.
...
- Cache entries are stored as one Brotli-compressed JSON file per key under `.agent-evals/cache/<sanitizedNamespace>/<keyHash>.json.br`, with a small namespace index sidecar at `.agent-evals/cache/<sanitizedNamespace>/.index-<namespaceHash>.json`. Listing and retention use the index without opening cached payloads. Index rows intentionally stay minimal: stored time, last access time, and external JSON blob refs. Each namespace is capped at `cache.maxBytes` indexed bytes, defaulting to `3 * 1024 * 1024` bytes. Retention counts cache payload files, raw-key debug sidecars, and referenced external JSON blobs without double-counting blobs within the namespace. Configure `cache.maxBytes` as a number for the default cap, or as `{ default, namespaces }` for exact namespace-specific caps. After a run finishes and the runner stays idle for `cache.pruneIdleDelayMs ?? 5000` milliseconds, retention removes cache referenced only by deleted evals, removes cache referenced only by non-latest saved runs older than `cache.oldRunMaxAgeMs ?? 1_296_000_000` milliseconds (15 days), then prunes older run-referenced entries before least-recently-accessed entries if the namespace is still over budget. Cache referenced by the latest saved run for each current eval is protected from automatic retention pruning. Cache cleanup logs each dropped namespace/key or cache file path to the terminal with the reason, including cleanup triggered by the web dev server after runs. Writes initialize the row's last access time to the stored time; later cache hits refresh that timestamp at the configured access-time update interval.
...
- Runs automatically apply branch-cache pruning before reporting completion, limited to cases that completed in that run. Only superseded durable entries added since the merge-base can be deleted. Cache referenced by untouched evals/cases (even their older runs), latest case hits/writes, base entries, unreferenced entries, and entries stored after the latest case run started are kept. Bypass runs and errored/cancelled cases do not supersede cache. Automatic cleanup skips when another run is active or Git/base resolution is unavailable, without failing the eval. Use the manual command below for workspace-wide cleanup or to retry skipped cleanup.
- Before merging a branch whose runs committed new cache entries, run `agent-evals cache prune-branch` (use `--dry-run` to preview, `--json` for a machine-readable summary). It removes durable cache entries added on the branch (not present at the merge-base) that saved runs referenced but the latest local run of each case no longer references. It never touches entries present at the merge-base, entries no saved run references, or entries stored after the latest run of a referencing case started. The base ref is `--base <ref>`, else the current PR's base branch from `gh pr view` (preferring `origin/<base>`), else `cache.branchPruneBaseRef` in config. It fails while a run is in progress.
