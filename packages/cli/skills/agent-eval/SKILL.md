...
- Before merging a branch whose runs committed new cache entries, run `agent-evals cache prune-branch` (use `--dry-run` to preview, `--json` for a machine-readable summary). It removes durable cache entries added on the branch (not present at the merge-base) that the latest local run of each case no longer references, and never touches entries present at the merge-base. The base ref is `--base <ref>`, else the current PR's base branch from `gh pr view` (preferring `origin/<base>`), else `cache.branchPruneBaseRef` in config. It fails without local run history or while a run is in progress, so re-run the affected evals first.
